...
```bash
# 创建名为  gpt2env 的新环境，指定  Python 版本  3.8 pip 不了 tensorflow1.x
...
```bash
python3 src/encode.py --input your_data.txt --output your_data.npz
```

微调模型

```bash
PYTHONPATH=src ./train.py --dataset your_data.npz --model_name 124M
```
...
```bash
python3 src/encode.py --input train_data.txt --output train_data.npz
```

微调  GPT-2 模型

```bash
PYTHONPATH=src ./train.py --dataset train_data.npz --model_name 124M
```
